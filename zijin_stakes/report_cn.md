...
> - **📊 第二部分（9月18日新增）—— 完整基本面 + 校准后目标价。核心发现是：业务质量与投资吸引力是"倒挂"的：**
>
> | | **赤峰黄金** | **招金矿业** | **Allied Gold** |
> |---|---|---|---|
> | 🐻 悲观（金价 $3,400） | −51% | **−21%** | 🔴 **−64%** |
> | ⚖️ **中性，12个月（金价≈现货）** | 🔴 **−14%** | **🟢 +40%** | **+68%** |
> | 🐂 乐观，24个月（金价 $5,200） | +6% | +129% | **🟢 +157%** |
> | EV / 年产量盎司 | 🔴 $21,746 | $14,911 | **🟢 $8,479** |
> | 净负债 | **🟢 净现金** | 🔴 +US$20.8亿 | 约 0 |
> | 风险调整后排序 | **第 3** —— 业务极好，价格已满 | **🟢 第 1** —— 结构最好（2:1） | **第 2** —— 上行最大，但有 −64% 尾部 |
>
> - **⚠️ 两处方法修正让这些数字变得可信。**（1）倍数**随金价上升而下调**（12x→10x→7x）—— 给牛市盈利套牛市倍数等于把乐观算两遍。（2）每个情景都**用 FY2025 实际数校准**：该模型对赤峰误差仅 **−5%**，但对 Allied 算出应赚 **+US$2.70亿**，而它实际**亏了 US$0.63亿** —— **3.33 亿美元**的折旧摊销、利息与成长性资本开支缺口，**AISC 完全看不见**。**AISC 不是利润。**
> - **仅供教育/分析，非投资建议。**
...
> 2026年9月18日应用户要求新增：三家各开一章的详细基本面分析，然后是优势/劣势/潜在收益/问题。建立在第一部分已核实的数据之上。**复现：`python run_fundamentals.py`。**

## §13 —— 第二部分的方法，以及我必须先修正的一个错误

### 13.1 三种相互独立的估值方法

对金矿股而言，任何单一方法都会骗人。第二部分同时用三种：

| # | 方法 | 它能抓住什么 | 它看不见什么 |
|---|---|---|---|
| **1** | **盈利桥** —— 金价 → 现金毛利 → 净利 → PE | 诚实的核心；从每家自己的产量与成本基数搭建 | 忽略副产品，采用统一税率 |
| **2** | **每盎司企业价值** —— EV ÷ 年产量、EV ÷ 储量盎司 | 当盈利模型被暂时的高金价粉饰时 | 需要储量披露（赤峰没有） |
| **3** | **自由现金流收益率** —— Day1Global 模块 C，"最重要的那个" | 无法把账面利润转化为现金的矿企 | 在大规模建设年份会失真 |

### 13.2 🔴 我犯的错误，以及修正

我第一次运行得出 **Allied Gold 牛市情景 +697%**。这是荒谬的，而原因值得写出来，因为它与本仓库 VLCC 总表里抓到的是同一个错误：

> **给牛市盈利套牛市倍数，等于把同一份乐观计算了两遍。**

周期性矿企的交易规律是：**盈利低迷时给高 PE，盈利见顶时给低 PE**（CRule 2）。因此下文修正后的情景**随金价上升而下调倍数**：**悲观 12x → 中性 10x → 乐观 7x。**

### 13.3 🔴 第二处修正：用现实校准

一个从未用实际业绩回测过的估值模型，就是个随机数发生器。2025 年金价均值约 **US$3,050/oz**，所以这个桥应该能复现 FY2025：

| 公司 | 模型算出的 FY2025 净利 | **实际 FY2025 净利** | 误差 | 判定 |
|---|---|---|---|---|
| 赤峰黄金 | US$437m | **US$460m** | **−5%** | ✅ 拟合良好 —— 其成本假设可信 |
| 招金矿业 | US$644m | **US$540m** | +19% | ✅ 可接受（模型忽略了低毛利的冶炼业务） |
| **Allied Gold** | **US$270m** | **🔴 −US$63m（亏损）** | **不适用** | **🔴 拟合极差 —— 3.33 亿美元缺口** |

**Allied 覆盖了 AISC 却依然亏钱。** 那 3.33 亿美元的缺口是折旧摊销、利息与成长性资本开支 —— **AISC 根本不包含它们**。因此下文每个情景都**减去了各公司自己的验证缺口**（赤峰 −US$23m、招金 +US$105m、Allied +US$333m）。**不做这个校准，Allied 的目标价就是幻想。**

> **这是第二部分最重要的方法论要点：AISC 不是利润；而对一家高成本、高折旧的矿企来说，这个差额就是全部投资逻辑所在。**

---

## §14 —— 赤峰黄金（600988.SS / 6693.HK）

### 14.1 这家公司到底是什么

| | |
|---|---|
| **资产** | 内蒙古三座矿山（吉隆 · 华泰 · 五龙，部分矿段 **>7 g/t**）+ **老挝 Sepon 万象矿业**（金*和铜*，约 350 万吨/年）+ **加纳 Wassa 金星瓦萨**（约 400 万吨/年） |
| **地域结构** | **2024 年产量的 74.2% 来自海外** —— 这不是一个中国内需故事 |
| **资源量** | **582.7 吨（+49.41%）** ✅ 已确认 |
| **储量** | **⚠️ 年报摘要未列表**（F3） |
| **紫金** | **25.85% 及控制权**，182.6 亿元，预计 **2026年9月30日**交割 |

### 14.2 财务 —— 三家中最强，而且差距明显

| 指标（人民币） | FY2022 | FY2023 | FY2024 | **FY2025** | 三年变化 |
|---|---|---|---|---|---|
| 营业收入 | 62.7亿 | 72.2亿 | 90.3亿 | **126.4亿（+40.0%）** | **2.0 倍** |
| 净利润 | 4.5亿 | 8.0亿 | 17.6亿 | **30.8亿（+74.7%）** | **🟢 6.8 倍** |
| 经营利润 | 9.8亿 | 14.7亿 | 29.3亿 | **52.1亿** | 5.3 倍 |
| 经营现金流 | — | — | — | **55.6亿（+70.0%）** | — |
| **负债/权益** | 50.5% | 45.9% | 28.9% | **🟢 8.3%** | **去杠杆** |
| 加权 ROE | — | — | 25.1% | **27.1%** | — |

**资产负债表：** 现金 **¥68.2亿** vs 总债务 **¥11.1亿** → **净现金 ¥57亿（US$8.5亿）**。三家中唯一的净现金公司。

**质量检验（Day1Global 模块 C —— 现金流，"最重要的那个"）：**

| | 数值 | 解读 |
|---|---|---|
| FY2025 经营现金流 | **¥55.56亿** | 对净利润 ¥30.82亿 → **OCF/净利 = 1.80 倍** ✅ 转化极好 |
| 自由现金流 | **¥25.06亿** | FCF 收益率 **3.4%** —— 偏低，因为仍在大建设期 |
| 分红 | **¥0.32/股**，股息率 **0.72%**，派息率 **16.2%** | 象征性。资本在留存 |
| EBITDA | **¥75.02亿**，EV/EBITDA **10.87x** | 三家中该指标最贵 |

> **模块 C 评级：A−。** 利润以 1.8 倍转化为现金。唯一扣分项是 FCF 收益率仅 3.4%，因为资本开支很重。
...
|---|---|---|
| FY2024 | **15.16 吨** | +5.6% |
| **FY2025** | **14.51 吨** | **🔴 −4.3% —— 未达标** |
| FY2026 计划 | **14.70 吨** | +1.4% |
| 2027 目标 | 约 20 吨 | ⚠️ *券商口径，未确认为公司指引* |

**H1-2026 的拆解 —— 这是本章最关键的表：**

| | H1-2026 | 变化 |
|---|---|---|
| 自产金**销量** | — | **🔴 −9.45%** |
| 自产金**售价** | ¥1,008.52/克 | **+44.08%** |
| 自产金**单位销售成本** | ¥383.44/克 | **🔴 +20.18%** |
| 自产金毛利率 | 61.63% | +7.11 pp |
| 营业收入 | ¥70.18亿 | +33.11% |
| 净利润 | ¥17.32亿 | +56.50% |

> **直白地读：利润涨了 56.5%，而销量跌了 9.45%。盈利增长的 100%（实际上超过 100%）来自金价。销量和成本两项都在与公司作对。**

**原因（已核实）：** **Wassa 矿石品位下降 + 雨季影响。** 成本驱动：资源税、权利金、与金价挂钩的可持续发展税费，以及低产量下更差的固定成本摊薄。

**而且 AISC 披露恰恰在成本上升时消失了：**

| | 数值 | 状态 |
|---|---|---|
| FY2023 AISC | **US$1,179.1/oz**（当年全球均值 **US$1,348.5**） | ✅ **已披露**于 H 股招股书 —— **真实**的成本优势 |
| FY2026 AISC | **⚠️ 未披露** | 券商估 ¥400–500/克 ≈ **US$1,750–2,190/oz** |

> **成本低的时候公司公布了一个好看的数字，成本上去之后就不公布了。** 这是一个正当的治理观察，不是指控 —— 但它意味着**目前没有任何外部人能核实赤峰的成本地位**，诚实的区间是 **US$1,179 → US$2,190**。

### 14.4 估值

| 指标 | A 股 600988 | H 股 6693 |
|---|---|---|
| 价格 | **¥44.26** | **HK$38.62**（= ¥32.91） |
| 市值 | **US$110.0亿** | 同一家公司 |
| 企业价值 | **US$101.4亿** | |
| PE 滚动/前瞻 | **22.4x / 12.8x** | **16.7x / 11.2x** |
| P/B | 5.82x | 4.35x |
| EV/EBITDA | 10.87x | |
| **每盎司年产量对应的 EV** | **🔴 US$21,746** | **三家中最贵，是 Allied 的 2.6 倍** |
| 券商目标价 | ¥50.29（+13.6%） | ⚠️ 仅 **3 位分析师** |
| **H 较 A 折价** | | **🟢 −25.6%** |

**情景目标价（已校准，采用周期性倍数纪律）：**

| 情景 | 金价 | AISC | 千盎司 | 净利 US$m | EPS ¥ | PE | **目标价 ¥** | **对 ¥44.26** |
|---|---|---|---|---|---|---|---|---|
| 🐻 悲观 | 3,400 | 2,190 | 473 | 452 | 1.82 | 12 | **21.82** | **−51%** |
| ⚖️ **中性（12个月）** | **4,400** | **1,800** | **473** | **944** | **3.80** | **10** | **38.01** | **🔴 −14%** |
| 🐂 乐观（24个月） | 5,200 | 1,800 | 643 | 1,663 | 6.69 | 7 | **46.84** | **+6%** |

> **令人不适的结论：三家中业务最好的那个，在当前价格上风险回报最差。** 连中性情景都是 **−14%**，而乐观情景 —— 金价 US$5,200 *且* 2027 年产量达到 20 吨 —— 也只有 **+6%**。**你在为一份正在萎缩产量的优质资产支付全价。**

### 14.5 优势 · 劣势 · 潜在收益 · 问题

**✅ 优势**
1. **资产负债表。** 净现金 ¥57亿；负债/权益从 50.5% 降至 **8.3%**。没人能逼它做任何事。
2. **真实的盈利能力。** ROE **27.1%**，经营利润率 **41%**，OCF/净利 **1.80 倍**。
3. **一个已被验证的历史成本优势** —— FY2023 AISC **US$1,179/oz**，低于 Agnico。
4. **资源增长是真的** —— **582.7 吨，+49.41%**。
5. **H/A 套利** —— 同一批资产，**港股便宜 25.6%**。
6. **紫金控股约在 2026年9月30日交割** —— 一个有"停滞—修复"验证记录的操盘方接手。

**❌ 劣势**
1. **🔴 产量在下滑** —— FY2025 −4.3%，H1-2026 −9.45%，FY2026 计划仅 +1.4%。
2. **🔴 增长 100% 来自价格。** 抽掉金价上涨，这是一家在萎缩的公司。
3. **🔴 成本 +20.18%，而 AISC 披露停止了。**
4. **所有资产口径指标最贵** —— 每盎司年产量 EV **US$21,746**，Allied 只有 US$8,479。
5. **覆盖太薄** —— 3 位分析师。
6. **储量未披露**（F3）。

**💰 潜在收益：** 中性 **−14%**，乐观 **+6%**。在这个价格上，现实的上行**不在**经营层面 —— 而在于（a）**25.6% 的 H/A 折价收敛**，以及（b）**紫金修好产量问题**，而那要到 2027 年产量上才看得见，不是 2026。

**⚠️ 需要盯的问题：** 9月30日交割；FY2026 能否达到或超过 14.7 吨计划；AISC 披露是否恢复；Wassa 品位。
...
| | |
|---|---|
| **资源量 / 储量** | **1,504.68 吨** / 可采 **521.24 吨** —— **三家中资源基础最大，遥遥领先** |
| **FY2025 产量** | 合计 **27.23 吨**（+2.93%）；**矿产金 19.79 吨**（+7.93%）；冶炼金 7.43 吨（−8.36%） |
| **真正重要的资产** | **海域金矿** —— 562 吨 @ **4.2 g/t**（最好矿段 6.6 g/t），可采约 212 吨，12,000 吨/日（396 万吨/年），**满产 15–20 吨/年**，服务年限约 23 年，总投资约 ¥60亿（2024年底已投约 ¥45亿） |
| **海域股权** | **招金 70% / 紫金 30%**（紫金 2022年10月付 ¥39.845亿） |
| **紫金在上市公司** | **18.20%**（2022年11月6日以 **HK$6.72** 入股，43.95 亿港元）；第一大股东为山东招金集团 **33.27%** |
| **紫金对海域的穿透权益** | **约 42.9%** |

**品位是关键：4.2 g/t 对 Allied 的 1.13–1.49 g/t。** 海域是一个真正优质的矿体。

### 15.2 🔴 H1-2026，直接读自港交所中期公告（2026年8月23日）

| 利润表（人民币千元） | H1 2026 | H1 2025 | 变化 |
|---|---|---|---|
| 营业收入 | **9,021,699** | 6,972,841 | **+29.38%** |
| 毛利 | 4,346,765 | 3,050,236 | +42.5%（**毛利率 48.18%**） |
| 其他收入及收益 | 146,912 | 1,133,655 | **🔴 −87.0%** |
| 期间利润 | 2,105,402 | 1,776,694 | +18.50% |
| **归属于母公司股东** | **1,578,089** | 1,439,690 | **🔴 +9.61%** |
| 少数股东损益 | 527,313 | 337,004 | **+56.5%** |
| 每股收益 | **¥0.42** | 0.38 | +10.75% |
| **中期股息** | **🔴 未派** | — | — |
...
> **一张表里三个红旗。**（1）归母利润仅增 **+9.61%**，而集团利润增 **+18.50%** —— 差额被少数股东拿走。（2）**其他收入减少 ¥9.87亿**，所以 +29% 的收入增速严重夸大了内生增长。（3）**矿产金 −21.87%** —— 国内基本盘被打断，原因为**国内矿山安全监管趋严**。

### 15.3 🔴 海域尚未投产 —— 而我能证明这个"缺席"

我获取并解析了招金 **2026年8月23日港交所中期业绩公告（32 页）**。

> **全文中"海域"二字出现 0 次。**

按港交所规则，公司最大在建项目投产通常需要披露。**这个沉默本身就是证据。** 二手来源称 **2026年2月一起安全生产事故**把投产从 2026年6月推迟到约 9月，**Q4-2026 产出首金（FY2026 约 1 吨）**，**2027 年 12–15 吨**。⚠️ **该延期原因仅有二手来源，本文按"据报道"处理，非断言。**

**佐证正是矿产金 −21.87%**：安全监管趋严、一起被报道的安全事故、国内产量塌陷 —— 三者构成**相互印证的连贯图景**。
...
> **这个治理标记很重要。** ¥53.9亿永续工具被记在**权益**里，但经济实质是**债**。剔除后归母权益降至约 ¥201.9亿，**真实杠杆显著高于 49.96% 所显示的水平。** 任何拿它的 P/B 去和赤峰、Allied 比较都会失真。
...
|---|---|
| 价格 | **HK$20.16** |
| 市值 / 企业价值 | **US$74.1亿 / US$94.9亿** |
| PE 滚动 / 前瞻 | **18.3x / 9.0x** |
| P/B | 3.02x ⚠️ *被永续工具粉饰* |
| EV/EBITDA | 10.15x |
| **每盎司年产量对应的 EV** | **US$14,911** |
| **每盎司储量对应的 EV** | **🟢 US$566 —— 三家中地下的盎司最便宜** |
| 券商目标价 | **HK$31.74（+57.4%）**，**9 位分析师，买入** |
| 分红 | 股息率 0.57%，派息率 9.9% |

**情景目标价（已校准）：**

| 情景 | 金价 | AISC | 千盎司 | 净利 US$m | EPS HK$ | PE | **目标价 HK$** | **对 HK$20.16** |
|---|---|---|---|---|---|---|---|---|
| 🐻 悲观 | 3,400 | 2,000 | 563 | 486 | 1.32 | 12 | **15.88** | **−21%** |
| ⚖️ **中性（12个月）** | **4,400** | **1,700** | **563** | **1,035** | **2.82** | **10** | **28.16** | **🟢 +40%** |
| 🐂 乐观（24个月，海域爬坡） | 5,200 | 1,700 | **965** | 2,427 | 6.61 | 7 | **46.25** | **🟢 +129%** |

⚠️ **招金的 AISC 是假设值 —— 它从未披露过。这是三个模型中最不可靠的一个。**

### 15.6 优势 · 劣势 · 潜在收益 · 问题

**✅ 优势**
1. **最好的矿体** —— 海域 **4.2 g/t**，对 Allied 的 1.13–1.49 g/t。
2. **最大的资源基础** —— 资源 1,504.68 吨，储量 521.24 吨。
3. **地下的盎司最便宜** —— 每储量盎司 EV **US$566**。
4. **真实的期权价值** —— 海域满产可贡献 **15–20 吨/年**，而公司 2025 年矿产金才 19.79 吨。**它可能让矿产金接近翻倍。**
5. **紫金在资产层面是同一利益方** —— 直接持有海域 30%，有十足动机推动它投产。
6. **已经被杀过估值** —— **距 52 周高点 −50.3%**，失望已相当程度计价。
7. **分析师覆盖最好** —— 9 位，买入，目标 +57.4%。

**❌ 劣势**
1. **🔴 海域仍未投产**，且公司未正式解释延期。
2. **🔴 存量基本盘在萎缩** —— 矿产金 **−21.87%**。
3. **🔴 盈利质量差** —— 归母 +9.61% vs 集团 +18.50%；其他收入 −87%。
4. **🔴 杠杆** —— 净负债 **¥139.3亿**，另有 **¥53.9亿永续工具伪装成权益**。
5. **未派中期息。**
6. **从不披露 AISC** —— 三家中唯一从未给过成本数字的。
7. **利润率最弱** —— 经营利润率 26%，赤峰 41%。

**💰 潜在收益：** 中性 **+40%**，乐观 **+129%**，下行 **−21%**。**三家中风险回报结构最好** —— 中性对下行约 **2:1**，乐观情景约 **6:1** —— *但完全取决于海域*。

**⚠️ 需要盯的问题：** **Q4-2026 首金这个说法，是整份报告里最重要的单一催化剂。** 另外：H2-2026 国内矿产金能否恢复；分红是否回归；永续工具是否再融资。

---

## §16 —— Allied Gold（AAUC.TO / NYSE American: AAUC）

### 16.1 本质：一个靠新矿翻身的高成本生产商

| | |
|---|---|
| **矿山** | **Sadiola**（马里）· **Bonikro + Agbaou**（科特迪瓦）· **Kurmuk**（埃塞俄比亚 —— 全新） |
| **FY2025 产量** | **379,081 盎司** —— 超过 >37.5 万盎司指引 |
| **FY2026 指引** | **485,000–575,000 盎司**（基础 38.5–42.5 万 + **Kurmuk 10–15 万**） |
| **储量（2026年2月 MRMR）** | Kurmuk 证实+概略 **2,706 千盎司 @ 1.32 g/t**；探明+控制（含 Kurmuk）**3,363 千盎司 @ 1.44 g/t**；推断 Sadiola 1,656 千盎司 @ 1.13 g/t |
| **紫金** | **约 9.2%**，价格 **C$32.55**（US$2.95亿），2026年8月10日前后完成 —— *C$44.00 收购失败后的安慰奖* |

### 16.2 🔴 核心事实：它位于全球成本曲线顶部
...
**而原因是结构性的，不是运营性的：品位只有 1.13–1.49 g/t。** 这是大吨位低品位组合。**品位是改不了的。** 对照海域的 4.2 g/t —— 约 **3 倍**。

### 16.3 财务，以及这个亏损为什么重要

| 指标（美元） | FY2024 | **FY2025** | **2026Q2** |
|---|---|---|---|
| 营业收入 | 约 7.29亿 | **13.3亿（+82.4%）** | 5.071亿 |
| **净利润** | **🔴 −1.156亿** | **🔴 −0.626亿** | **+0.372亿**（调整后 0.554亿） |
| 调整后 EBITDA | — | — | **1.650亿** |
| 现金 | — | — | 1.922亿 |
| 总债务 | — | — | 1.954亿 |
| **净现金 /（净负债）** | — | **+3.10亿 净现金** | **🔴 约 −0.032亿（基本持平）** |

> **半年之内从 3.1 亿美元净现金变成基本持平，摆动 3.13 亿美元。** 管理层归因于 Kurmuk 建设资本开支高峰，并指引 H2-2026 现金回升（资本开支下降、产量爬坡，加上 **US$2.95亿紫金定增**落地 —— 该笔在 6月30日资产负债表日**之后**完成，因此**不在** Q2 数字里）。⚠️ *2025年底净现金与 2026Q2 债务数字在我的来源中未完全调平。*

**模块 C（现金流）：** 经营现金流 **US$3.60亿**，自由现金流 **US$1.50亿**，**FCF 收益率 4.7%** —— **三家中最高**。这是真正的多头论据：即使会计利润为负，现金是真的。

**但模型验证是致命的：** 按 2025 年平均金价，这个桥算出 Allied 应该赚 **+US$2.70亿**；它实际**亏了 US$0.63亿**。**3.33 亿美元的折旧摊销、利息与成长性资本开支缺口，AISC 完全看不见。** 评级：**模块 C = B−，模块 O（会计质量）= C** —— 因为 AISC 实质性低估了这家公司的真实成本。

### 16.4 ✅ 公允地陈述多头逻辑：Kurmuk

**Kurmuk 距首金只差几天。** 输电线路**已带电**；**2026年9月初首批矿石已进入破碎回路**。目标 AISC **<US$1,200/oz，甚至 <US$1,000/oz**。

**关键算术（我的计算，非公司指引）：**

| | 盎司 | AISC |
|---|---|---|
| 存量基础 | 约 40 万 | 约 US$2,250 |
| Kurmuk | 约 24–27 万 | **<US$1,200** |
| **混合后** | **约 65 万** | **≈ US$1,700/oz** |

> **那仍将高于巴里克 —— 但它会把这家公司从"位于成本曲线顶部、不可投资"变成"一家普通的高成本生产商"。** 真正的投资逻辑是这次重估，而不是金价。

⚠️ **时点风险就在眼前：** 首金要到 9 月中下旬，**Kurmuk 10–15 万盎司的指引上限已不可能实现。** 485–575 千盎司的集团指引应按**下半区**看待。
...
|---|---|
| 价格 | **C$32.30** |
| 市值 / 企业价值 | **US$32.1亿 / US$32.1亿** |
| PE 滚动 | **不适用（亏损）** |
| PE 前瞻 | **🟢 3.2x** —— 市场在给一次阶跃式变化定价 |
| P/B | 5.81x |
| EV/EBITDA | **7.27x** —— 三家中最便宜 |
| **每盎司年产量对应的 EV** | **🟢 US$8,479 —— 最便宜，差距 1.8 倍** |
| FCF 收益率 | **🟢 4.7%** —— 三家最高 |
| 券商目标价 | **C$44.60（+38.1%）**，6 位分析师，买入 |
| 分红 | **无** |

**情景目标价（已校准 —— 注意已减去 3.33 亿美元缺口）：**

| 情景 | 金价 | AISC | 千盎司 | 净利 US$m | EPS C$ | PE | **目标价 C$** | **对 C$32.30** |
|---|---|---|---|---|---|---|---|---|
| 🐻 悲观 | 3,400 | 2,264 | 505 | **98** | 0.98 | 12 | **11.78** | **🔴 −64%** |
| ⚖️ **中性（12个月）** | **4,400** | **2,100** | **505** | **538** | **5.42** | **10** | **54.16** | **🟢 +68%** |
| 🐂 乐观（24个月，Kurmuk 满产） | 5,200 | 2,100 | 650 | 1,179 | 11.86 | 7 | **82.99** | **🟢 +157%** |

> **看悲观情景：−64%。** 金价 US$3,400、AISC US$2,264 时，校准后净利润塌缩到 **US$0.98亿** —— 公司几乎不赚钱。**这就是"位于成本曲线顶部"的含义。** Allied 上行最大，下行也远远最大。

### 16.6 优势 · 劣势 · 潜在收益 · 问题

**✅ 优势**
1. **所有资产口径指标最便宜** —— 每盎司年产量 EV **US$8,479**，EV/EBITDA **7.27x**。
2. **FCF 收益率最高（4.7%）**，尽管账面亏损 —— 现金是真的。
3. **🔴→🟢 Kurmuk 是催化剂，而且迫在眉睫** —— 2026年9月已投料，目标 **<US$1,200/oz**。
4. **对金价的经营杠杆最高 —— 弹性 2.04x**，同业中最高。
5. **紫金几周前刚以 C$32.55 验证过价格**，而紫金会做自己的尽调。
6. **马里风险低于标题所暗示的** —— Allied 与军政府**早期低价和解**，不像巴里克在被捕、扣押、实质国有化之后支付了**约 US$4.3亿**。
7. **券商目标 C$44.60（+38%）**，6 位分析师。

**❌ 劣势**
1. **🔴 2026Q1 AISC US$2,264/oz —— 比 Agnico 高 69%。** 全球成本曲线顶部。
2. **🔴 结构性低品位（1.13–1.49 g/t）** —— 这改不了。
3. **🔴 FY2024 与 FY2025 连续两年亏损。**
4. **🔴 模型缺口：3.33 亿美元 AISC 看不见的成本。**
5. **半年内从净现金变为持平。**
6. **马里 + 埃塞俄比亚司法辖区风险** —— 军政府的剧本（审计 → 税务索赔 → 扣押 → 和解）**可重复**。
7. **FY2026 指引大概率落在下半区**，因 Kurmuk 时点。
8. **无分红。**
9. **🔴 它已经被否决过一次** —— 发改委否决了紫金 C$44.00 的收购，据报理由包括溢价**以及马里风险**。**一个成熟买家自己的政府，在 C$44 说了不。**

**💰 潜在收益：** 中性 **+68%**，乐观 **+157%**，下行 **−64%**。**期望回报最高，方差也最高。** 中性对下行约 **1:1** —— 尽管标题上行更大，**风险回报结构远差于招金**。

**⚠️ 需要盯的问题：** Kurmuk 实际爬坡 vs <US$1,200/oz 目标；2027 年集团 AISC 能否降到 US$1,800 以下；马里；FY2026 是否低于 48.5 万盎司。
...
| | **赤峰黄金** | **招金矿业** | **Allied Gold** |
|---|---|---|---|
| **一句话** | 业务最好，价格最差 | 押注单一矿山的期权 | 资产最便宜，成本最高 |
| 市值（美元） | 110.0亿 | 74.1亿 | **32.1亿** |
| 净负债（美元） | **🟢 −8.5亿（净现金）** | 🔴 +20.8亿 | 约 0 |
| FY2025 净利率 | **🟢 +24.4%** | +20.0% | 🔴 −4.7% |
| PE 滚动 / 前瞻 | 22.4x / 12.8x | 18.3x / 9.0x | 不适用 / **3.2x** |
| EV/EBITDA | 10.87x | 10.15x | **🟢 7.27x** |
| **EV / 年产量盎司** | 🔴 **US$21,746** | US$14,911 | **🟢 US$8,479** |
| EV / 储量盎司 | 未披露 | **🟢 US$566** | US$1,186 |
| FCF 收益率 | 3.4% | 未披露 | **🟢 4.7%** |
| AISC | ⚠️ 1,179（FY23）→ 停止披露 | ⚠️ 从未披露 | 🔴 **2,264 已披露** |
| 品位 | 国内矿段 >7 g/t | **🟢 4.2 g/t（海域）** | 🔴 1.13–1.49 g/t |
| 模型验证误差 | **🟢 −5%** | +19% | 🔴 **拟合极差（3.33亿缺口）** |
| **悲观** | −51% | **🟢 −21%** | 🔴 −64% |
| **中性（12个月）** | 🔴 −14% | **🟢 +40%** | +68% |
| **乐观（24个月）** | +6% | +129% | **🟢 +157%** |
| 2014年来最大回撤 | −70% | −60% | −51% |
| 距 52 周高点 | −10.5% | 🔴 −50.3% | −31.2% |
| 分析师覆盖 | 3 | **🟢 9（买入）** | 6（买入） |

### 17.2 结论

> **最有用的一个发现：在这三家身上，业务质量与投资吸引力是"倒挂"的。**

**赤峰的业务质量遥遥领先** —— 净现金、27% ROE、1.8 倍现金转化、经验证的历史成本优势。**而它的风险回报最差**，因为市场已经为这一切付过钱了（**每盎司年产量 EV US$21,746**，是 Allied 的 2.6 倍），而**产量在下滑**。中性情景 **−14%**。

**招金的风险回报结构最好** —— 下行 **−21%** 对中性 **+40%**、乐观 **+129%**，且拥有最好的矿体（4.2 g/t）与最便宜的储量盎司（US$566）。**但整个逻辑压在一座尚未投产、且公司未解释延期原因的矿上。**

**Allied 上行最大，窟窿也最大** —— 中性 **+68%**，乐观 **+157%**，悲观 **−64%**。它便宜是有原因的：**位于全球成本曲线顶部、品位无法改善、连续两年亏损、AISC 与现实之间有 3.33 亿美元的缺口。**

**按风险调整后吸引力的排序 —— 明确标注为解释，而非事实：**

| 排名 | 标的 | 理由 | 决定性的那一件事 |
|---|---|---|---|
| **1** | **招金矿业** | 结构最好（中性/下行 2:1）、品位最好、储量最便宜、已跌 50%、9 位分析师买入 | **海域能否在 2026Q4 产出首金？** |
| **2** | **Allied Gold** | 资产最便宜、FCF 收益率最高、催化剂迫在眉睫 —— 但下行 −64% | **Kurmuk 能否真的做到 <US$1,200/oz？** |
| **3** | **赤峰黄金（只买 H 股）** | 业务极好，但已充分定价。**若要买，买折价 25.6% 的 H 股，绝不买 A 股** | **紫金能否在 2027 年扭转产量下滑？** |

### 17.3 事前验尸："一年后我亏了 40%，发生了什么？"

| # | 情景 | 概率* | 早期预警 |
|---|---|---|---|
| 1 | **金价跌到 US$3,400。** 三家全部重挫；Allied −64%，赤峰 −51% | **25%** | 美国实际利率上升；[沃什的美联储](../volcker_warsh/report_cn)继续加息 |
| 2 | **海域再次推迟到 2027 年** | **30%** | FY2026 业绩前仍无投产公告；又一次安全整顿 |
| 3 | **Kurmuk 爬坡缓慢 / AISC 高于 US$1,500** | **30%** | 2026Q4 与 2027Q1 的 AISC 数据；FY2026 低于 48.5 万盎司 |
| 4 | **马里或埃塞俄比亚对 Allied 出手** | **15%** | 新的税务审计、出口限制、采矿权复审 |
| 5 | **赤峰 2026 年产量再次下滑** | **35%** | 季度产量；Wassa 品位表述 |
| 6 | **紫金的赤峰交易未能交割** | **10%** | 2026年9月30日之后仍无消息 |
...
| 偏见 | 在这里会怎样咬人 | 已采取的缓解 |
|---|---|---|
| **叙事偏见** | "跟着紫金"是个诱人的故事 | §6 显示西部矿业在紫金**被动**持股下也涨 6.5 倍 —— 故事可能只是 beta |
| **锚定偏见** | 锚定紫金的入股价（HK$6.72、C$32.55） | 那是 **2022 年与 2026 年**、不同金价体制下的价格 |
| **近因偏见** | US$4,416 的金价感觉是永久的 | 悲观情景刻意使用 **US$3,400** |
| **确认偏见** | 希望藏格 15 倍的模式重演 | **招金结构完全相同，却没有重演** |
| **幸存者偏见** | 名单由"发生过的交易"构成 | Xanadu 与 Solaris 被纳入，**正因为**它们失败了 |
| **简化偏见** | "AISC 就是成本" | **Allied 的 3.33 亿美元缺口证伪了它** |
...
python run_fundamentals.py     # 第二部分：基本面、每盎司EV、校准后目标价
```
...
| `data/fundamental_snapshot.csv` | §14–16 以美元归一的快照 |
| `data/ev_per_ounce.csv` | 每年产量盎司与每储量盎司的 EV |
| `data/model_validation.csv` | **校准全部目标价的 FY2025 回测** |
| `data/target_prices.csv` | 完整的 Rule 7 情景网格 |
| `charts/follow_zijin.png` · `company_deep.png` · `gold_leverage.png` · `fundamentals.png` | 四张图 |
...
| **海域如报道所言在 2026Q4 产出首金** | §15.3 的反向发现 —— 那会让招金成为明确的第一 |
| **Kurmuk 头两个季度的 AISC 低于 US$1,200/oz** | §16.2 的"结构性高成本" —— Allied 会大幅重估 |
| 赤峰恢复披露 AISC 且低于 US$1,500/oz | §14.3 的"成本上升时披露停止了" |
