...
## Prompt 75：BWET（油轮运价 ETF）对 DHT/FRO 的相关性分析 —— 又四项阻断级意见、核心结论撤回，并倒逼更正 §21

**用户原话：**「我又发现一个有意思的东西。对bwet和dht/fro的股价走势做分析，看一看相关性有多少；把这些做进同一张表。」

BWET 是 Breakwave 油轮运价 ETF，持有油轮**运费期货**，因此是最接近运价本身的可交易代理，天然适合检验「运价波动有多少传导到股票」。

**在评审返回之前，我自己先抓到两个错误。** 其一，用简单累计收益之比计算「捕获率」得到 DHT 4.2% —— 在 BWET 上涨 59 倍时这毫无意义，因为结果被复利效应主导；我改用对数收益（30.2%/38.4%）。其二，我用 4000 次自助法检验上下行捕获的不对称性，发现两个区间都跨越零，于是在发布前撤回了自己的「有利不对称性」论点。

**🔴 随后 Astra 返回四项阻断级意见。**

1. **[A1] 一个真实的算术 bug。** 分段收益按日历切片取首末值计算，**丢掉了每个分段边界上的收益**。分段复利乘积无法还原全期收益 —— BWET 偏 +2.16%、FRO 偏 +2.42%。我独立验证后改为按日收益复利重建，现误差 0.000000%。更正后 2026 年至今：BWET **+4,229%**（原 +4,310%）、DHT **+108%**（原 +117%）、FRO **+162%**（原 +178%）。
2. **[A2] 该错误回溯影响了 §21。** 我写的「DHT 与 FRO 只赚到 TD3C 印刷值的 19%/18%」，是用 **Q2 平均**实际运价除以 **9 月 11 日**的评估值。一家公司完全可能赚到同期 Q2 基准的 100%，却仍显示约 19%。**§21 已就地更正。**
3. **[A3] 核心结论属于数字凑巧。** 我曾声称股票的对数捕获率与经营捕获率「落在同一区间」，从而证明市场正确定价了可转化份额与久期。撤回：两者是不同对象（滚动期货组合的投资收益 vs 运价水平之比）、期间不同，且改用简单收益后「吻合」彻底消失（2.6%/3.8%）。FRO 的 25.6% 也比它「印证」的 18% 高出约 42%。
4. **[A4]「30.3%/38.5% 是正确的捕获率」—— 撤回。** Astra 的反例具有决定性，现已写入脚本实算：把 DHT 的日收益随机洗牌，对数增长比**纹丝不动仍为 30.2%**，而它与 BWET 的相关性从 0.317 崩塌到 0.014。一个能在「摧毁与 BWET 全部联系」的洗牌中存活的统计量，不含任何关于传导的信息。

另有八项重大意见全部接受：相关性强度不能识别因子主次；相关性随周期上升与 Epps 效应一致但不具诊断性；低 R² 与精确 beta 可以并存且 beta 不等于经营传导；上下行差距仅凭一个正截距即可产生（而此处两个 alpha 确实都为正）；「无择时优势」收窄为「未确立周频线性领先」；周/月样本含不完整末期（现为 176 与 39）；危机期相关性上升**不显著**（p=0.360 与 p=0.075）；以及「无拆分」不足以验证 BWET 是干净基准 —— 其招募说明书显示约 90% TD3C/10% TD20、50–70 天目标期限、3.50% 费率、溢价/折价曾达 ±7%。

**留下来的结论。** 两只股票彼此相关 0.84–0.87，而与运价工具仅 0.32–0.51，在每个频率上经相依相关的 Williams 检验都极其显著（日频 p=3.1e-108）。对 BWET 的 beta 仅 0.144/0.186，R² 约 0.10 —— 股价日波动约 90% 不由运价驱动。领先滞后曲线均在 k=0 达峰，且无任何非零滞后能通过 Holm 校正。实务读法：同时持有 DHT 与 FRO 接近于一个仓位而非分散；若你要的是运价敞口，这两只股票每单位风险提供的运价暴露非常少。

**创建文件**: vlcc_cycles/run_bwet_correlation.py, data/s22_*.csv (7), charts/s22_bwet.png
**更新文件**: vlcc_cycles/report_en.md, report_cn.md（新增 §22、更正 §21）, Prompt_Log_EN.md, Prompt_Log_CN.md
